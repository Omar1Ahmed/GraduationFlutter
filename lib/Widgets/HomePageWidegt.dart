...
class _HomePageState extends State<HomePage> {
          ...
          child: Stack(
            children: [
              ...
              _RadioSelected == 0 ? Container(
                ...
                margin: isEnglish() ? EdgeInsets.only(
                    left: ScreenWidth * 0.059, top: ScreenHeight * 0.15) :
                EdgeInsets.only(
                    left: ScreenWidth * 0.50, top: ScreenHeight * 0.15),
                ...
                child: Row(
                  ...
                  children: isEnglish() ? [
                    ...
                    Container(
                      margin: EdgeInsets.only(right: ScreenWidth * 0.071,
                      ...
                      child: PopupMenuButton(
                        offset: Offset(
                            ScreenWidth * 0.01, ScreenHeight * 0.023),

                        popUpAnimationStyle: AnimationStyle(curve: Curves
                            .easeInCirc, reverseCurve: Curves.easeInCirc),
                        iconSize: ScreenWidth * 0.04,
                        position: PopupMenuPosition.over,
                        color: Colors.black,
                        icon: const Icon(Icons.settings_suggest, color: Color(
                            0xFF8C7EBE),),
                                  ...
                                  children: [
                                    ...
                                    SizedBox(width: ScreenWidth * 0.09,
                                        ...
                                        child: TextField(
                                            onSubmitted: (value) {
                                              if (value.isNotEmpty) {
                                                widget.Nearest!.setInt(
                                                    'Days', int.parse(value));
                                                NearestMeetingDayDate =
                                                    DateTime.now()
                                                        .add(Duration(
                                                        days: int.parse(value)))
                                                        .toString()
                  ...
                  [
                    Container(
                      margin: EdgeInsets.only(right: ScreenWidth * 0.011,
                          bottom: ScreenHeight * 0.022),
                      width: ScreenWidth * 0.059,
                      height: ScreenHeight * 0.022,
              ...
              Container(
                margin: EdgeInsets.only(
                    left: ScreenWidth * 0.87, top: ScreenHeight * 0.030),
                child: IconButton(
              ...
              Container(
                ...
                margin: _RadioSelected == 0 ? EdgeInsets.only(
                    ...
                    right: ScreenWidth * 0.071,
                    top: ScreenHeight * 0.193) : EdgeInsets.only(
                    left: ScreenWidth * 0.071,
                    right: ScreenWidth * 0.071,
                ...
                RefreshIndicator(
                  ...
                  child: FutureBuilder(
                      future: widget.calenderDataBool ? RadioSelectedSetter(
                          getDays()) : fetchData(),
                            ...
                            itemBuilder: (context, index) {
                              if (widget.calenderDataBool == true) {
                                if (snapshot.hasData) {
                                  ...
                                  return CardVu(
                                    MeetingId: snapshot
                                        .data![index]['meeting_id'].toString(),
                                    PersonOrEntity_title: snapshot
                                        .data![index]['person'],
                                    Topic_Content: snapshot
                                        .data![index]['about']
                                        .toString(),
                                ...
                                return CardVu(
                                  MeetingId: snapshot.data![index]['meeting_id']
                                      .toString(),
                                  PersonOrEntity_title: snapshot
                                      .data![index]['person'],
              ...
              Container(
                ...
                height: ScreenHeight * 0.056,
                margin: EdgeInsets.only(
                    left: ScreenWidth * 0.094, top: ScreenHeight * 0.033),
                alignment: Alignment.center,
                decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(ScreenHeight * 0.056),
                    //50
                ...
                child: InkWell(
                  ...
                  child: Text(
                    accData.getString('userName')!.toUpperCase().substring(
                        0, 1),
        ...
        child: Column(
          ...
          children: [
            _RadioSelected == index ? Container(
              ...
              child: Padding(
                  padding: EdgeInsets.only(left: ScreenWidth * 0.0094,
                      right: ScreenWidth * 0.0094,
                      top: ScreenHeight * 0.0045,
                      bottom: ScreenHeight * 0.0045),
                  child: Icon(
                    icon,
                    color: const Color.fromRGBO(50, 213, 131, 100),
  ...
  Future<List<Map>> fetchData() async {
    cardData = isChecked
        ...
        'SELECT * FROM Meetings where meeting_id in(select meeting_id from meeting_Manager where manager_id = ${accData
            .getString(
            'managerId')}) and date >= \'$currentDayDate\' and date <= \'$NearestMeetingDayDate\' order by date ')
        :
    await widget.sqldb.readData(
        'SELECT * FROM Meetings where meeting_id in(select meeting_id from meeting_Manager where manager_id = ${accData
            .getString('managerId')}) order by date');
    ...
    if (await api.hasNetwork()) {
      ...
      if (cardData.isEmpty) {
        Response = isChecked ? await api.getRequest(
        ...
        var readData = isChecked ? await widget.sqldb.readData(
            'select max(createdAt) as maxCreated, max(updatedAt) as maxUpdated from Meetings where meeting_id in( select meeting_id from meeting_Manager where manager_id = ${accData
                .getString(
                'managerId')} and date >= \'$currentDayDate\' and date <= \'$NearestMeetingDayDate\')')
            : await widget.sqldb.readData(
            'select max(createdAt) as maxCreated, max(updatedAt) as maxUpdated from Meetings where meeting_id in( select meeting_id from meeting_Manager where manager_id = ${accData
                .getString('managerId')} )');

        maxCreatedDate = DateTime.parse(readData[0]['maxCreated']).add(
            const Duration(seconds: 1)).toString();

        Response = isChecked ? await api.getRequest(
            'https://meetingss.onrender.com/meetings?sort=date&isUpdated=false&date[lte]=$NearestMeetingDayDate&date[gte]=$currentDayDate&createdAt[gte]=${DateTime
                .parse(readData[0]['maxCreated']).add(
                const Duration(seconds: 1)).toString()}',
            {'token': '${loginInfo.getString('token')}'})
            : await api.getRequest(
            'https://meetingss.onrender.com/meetings?sort=date&isUpdated=false&createdAt[gte]=${maxCreatedDate}&date[gte]=$currentDayDate',
            {'token': '${loginInfo.getString('token')}'});
        ...
        Response = isChecked ? await api.getRequest(
            'https://meetingss.onrender.com/meetings?sort=date&isUpdated=true&date[lte]=$NearestMeetingDayDate&date[gte]=$currentDayDate&updatedAt[gte]=${DateTime
                .parse(readData[0]['maxUpdated']).add(
                const Duration(seconds: 1)).toString().substring(0, 19)}',
            {'token': '${loginInfo.getString('token')}'})
            : await api.getRequest(
            'https://meetingss.onrender.com/meetings?sort=date&isUpdated=true&date[gte]=$currentDayDate&updatedAt[gte]=${DateTime
                .parse(readData[0]['maxUpdated']).add(
                const Duration(seconds: 1)).toString().substring(0, 19)}',
            {'token': '${loginInfo.getString('token')}'});
      ...
      Response = await api.getRequest(
          'https://meetingss.onrender.com/meetings/getMeetingManagers?',
          {'token': '${loginInfo.getString('token')}'});


      List<Map> lol = await widget.sqldb.readData(
          'select meeting_id from meetings where meeting_id in (select meeting_id from meeting_Manager where manager_id = ${accData
              .getString(
              'managerId')}) and date >= \'$currentDayDate\'  order by meeting_id');
      ...
      int Localcount = (await widget.sqldb.readData(
          'select count(meeting_id) as count from meetings where meeting_id in (select meeting_id from meeting_Manager where manager_id = ${accData
              .getString('managerId')}) and date >= ${DateTime.now()
              .toString()
              .substring(0, 10)} '))[0]['count'];
      int onlineCount = api
          .getValue(Response, 'meeting_id')
          .length;

      if (Localcount != onlineCount) {
        for (int loop = 0; loop < onlineCount; loop++) {
          if (!ids.contains(lol[loop]['meeting_id'].toString())) {
            await widget.sqldb.deleteData(
                'delete from meetings where meeting_id = ${lol[loop]['meeting_id']}');
            await widget.sqldb.deleteData(
                'delete from meeting_Manager where meeting_id = ${lol[loop]['meeting_id']}');
            deletion = true;
          }
        }
        Localcount = (await widget.sqldb.readData(
            'select count(meeting_id) as count from meetings where meeting_id in (select meeting_id from meeting_Manager where manager_id = ${accData
                .getString('managerId')}) and date >= ${DateTime.now()
                .toString()
                .substring(0, 10)} '))[0]['count'];
        ...
        if (Localcount > onlineCount) {
          for (int loop = 1; loop <= Localcount - onlineCount; loop++) {
            await widget.sqldb.deleteData(
                'delete from meetings where meeting_id = ${lol[lol.length -
                    loop]['meeting_id']}');
            await widget.sqldb.deleteData(
                'delete from meeting_Manager where meeting_id = ${lol[loop]['meeting_id']}');
    ...
    if (insertion || update || deletion) {
      cardData = isChecked ? await widget.sqldb.readData(
          'SELECT * FROM Meetings where meeting_id in(select meeting_id from meeting_Manager where manager_id = ${accData
              .getString(
              'managerId')}) and date >= \'$currentDayDate\' and date <= \'$NearestMeetingDayDate\' order by date ')
          : await widget.sqldb.readData(
  ...
  Future<void> insertDataToLocalDb(String Response) async {
    if (api.getValue(Response, 'count')[0] != '0') {
      ...
      for (int i = 0; i < meetingIds.length; i++) {
        await widget.sqldb.insertData(
            'INSERT INTO Meetings(meeting_id, time, date, about, in_or_Out, address, notes, person, updatedAt, createdAt, attachmentLink, attachmentName, status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
            [
              meetingIds[i],
              meetingTimes[i],
              meetingDates[i],
        ...
        await widget.sqldb.insertData(
            "insert into meeting_Manager (meeting_id, manager_id) values (?,?)",
            [
              meetingIds[i],
    ...
    if (api.getValue(Response, 'count')[0] != '0') {
      ...
      for (int i = 0; i < meetingIds.length; i++) {
        await widget.sqldb.updateData(
            'update Meetings set meeting_id = ?, time = ?, date = ?, about = ?, in_or_Out = ?, address = ?, notes = ?, person = ?, updatedAt = ?, attachmentLink = ?, attachmentName = ?, status = ? where meeting_id = ?',
            [
              meetingIds[i],
              meetingTimes[i],
              meetingDates[i],
  ...
  void nearsetSharedPref() async {
    ...
    NearestMeetingDayDate =
        DateTime.now().add(Duration(days: widget.Nearest!.getInt("Days")!))
            .toString()
            .substring(0, 10);
  }
  ...
  CalenderVuWidget(List<DateTime> Dates) {
    ...
    for (int loop = 0; loop < Dates.length; loop ++) {
      if (loop == 0) {
        ...
        widget.DateWithCounter[widget.DateWithCounter.length - 1]['Date'] =
        Dates[loop - 1];
        widget.DateWithCounter[widget.DateWithCounter.length - 1]['counter'] =
            widget.DateWithCounter[widget.DateWithCounter.length -
                1]['counter'] + 1;
      } else {
    ...
    return Column(
      ...
      children: [
            ...
            child: CalendarDatePicker2(
                  ...
                  for (int loop = 0; loop < value.length; loop ++) {
                    ...
                    if (Dates.contains(value[loop])) {
                      widget.counter = (widget.counter +
                          widget.DateWithCounter[widget.DateWithCounter
                              .indexWhere((element) =>
                          element['Date'] == value[loop])]['counter']) as int;
                    }
                  }
              ...
              config: CalendarDatePicker2Config(
                ...
                nextMonthIcon: const Icon(
                  Icons.arrow_forward_ios_sharp, color: Color(0xff7E7EBE),),
                lastMonthIcon: const Icon(
                  Icons.arrow_back_ios_sharp, color: Color(0xff7E7EBE),),
                ...
                dayBuilder: (
                  ...
                  for (int loop = 0; loop <
                      widget.DateWithCounter.length; loop ++) {
        ...
        Container(
            ...
            child: ValueListenableBuilder(valueListenable: widget.number,
              builder: (BuildContext context, value, Widget? child) {
                if (value != 0) {
                  return Row(
                      mainAxisAlignment: MainAxisAlignment.spaceAround,
                      children: [
                        Text(S.of(context).totalMeetings(value),
