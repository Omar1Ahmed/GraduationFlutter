      ...
      child: SingleChildScrollView(
          child: Center(
            child: Column(
              children: [
                Container(
                  margin: EdgeInsets.only(top: screenSize.height * 0.02, bottom: screenSize.height * 0.0),
                ...
                Container(
                  ...
                  margin: EdgeInsets.only(top: screenSize.height * 0.05, bottom: screenSize.height * 0.01),
                  child: Form(
                    key: ValidateEmail,
                    child: Material(
                      elevation: 10,
                ...
                Container(
                  width: screenSize.width * 0.85,
                  margin: EdgeInsets.only(top: screenSize.height * 0.02, bottom: screenSize.height * 0.02),
                  child: Form(
                    key: ValidatePass,
                    child: Material(
                      elevation: 10,
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    style: ButtonStyle(
                      backgroundColor: MaterialStateProperty.all(const Color(0xff7E7EBE)),
                      elevation: MaterialStateProperty.all(6),
                    ),
                    onPressed: () async {
                      if (await api.hasNetwork()) {
                        ...
                        if (ValidateEmail.currentState!.validate() &&
                          ...
                          var Response = await api.postRequest(
                              ...
                              jsonEncode({
                                txtMail.text.contains('@') ? "E_mail" : "UserName": txtMail.text,
                                "PassWord": PassTxt.text,
                                "role": 'Manager',
                              }));
                          ...
                          if (api.getValue(Response, 'success')[0] == 'true') {
                            if (checkedValue) {
                              loginInfo.setString('token', api.getValue(Response, 'token')[0]);
                            ...
                            if (api.getValue(Response, 'success')[0] == 'true') {
                              accData.setString(
                                  'managerId', api.getValue(Response, 'manager_id')[0]);
                              accData.setString(
                                  'firstName', api.getValue(Response, 'first_name')[0]);
                              accData.setString(
                                  'lastName', api.getValue(Response, 'last_name')[0]);
                            ...
                            Navigator.of(context).pushReplacement(
                                MaterialPageRoute(builder: (context) => Grad(accData, loginInfo)));
                          } else if (api.getValue(Response, 'message')[0].contains('Email') ||
                              api.getValue(Response, 'message')[0].contains('UserName')) {
                            invalidMail = true;
                            ValidateEmail.currentState!.validate();
                          } else {
                            invalidPass = true;
